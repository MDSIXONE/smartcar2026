...
`mission` 启动前必须先确认**车已放回起点**：脚本会询问「是否已把车放回起点」，
输入 `yes` 才继续，其他输入直接取消启动。因为定位初值固定为起点
`(-0.25, 2.75, 0)`，车辆不在起点时启动会产生错误的地图位姿（曾因此发生碰撞）。

任务节点启动后（`WAITING_FOR_ITEM` 状态）会等待物品输入：在启动 `start_2026.sh`
的同一终端直接输入本次放入的物品名并回车（如：苹果、可乐、螺丝刀）；输入为空将
中止任务。

`mission` 模式不启动 RViz，执行 `52 → 面向 262 扫码一次 → 物品文本经星火分类得到
目标加工类别 → 12→22→13→23→14→24→15→25→16→26→17→27→18→28→19→29 逐点
360° OCR 转圈，首次命中目标类别即停 → 加工区停车点（墙交点向场内 25 cm，如
300→点 7、455→点 20）停留 3 s → 终点 17 朝向 300 → SUCCEEDED`。
每个生产点都沿下一状态点方向拍照；OCR 框通过纯旋转对齐画面水平中心后，用新鲜前向
雷达和扫描时刻 TF 匹配 JSON 墙点。非目标类别候选不停车继续转圈（`IGNORED`）。
历史 yolo2025 流程仍保留为 `full` 模式，但不属于
...
`~/.ros/ucar_2026_observations/run_*/observations.json` 保存目标类别的墙点及 OCR
内容（结果 JSON 含 `item_text`/`target_category`）。
