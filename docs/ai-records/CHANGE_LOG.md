...
## 2026-08-20｜独立 OCR 节点同步车端（改动完成）

- **目标**：将本轮新增的独立 OCR 搜索/连续对齐运行文件同步到车端，并补齐 Python 文件执行权限。
- **同步内容**：`ocr_search.py`、`ocr_alignment.py`、`ocr_search.launch`、`ocr_alignment.launch`、两个独立入口测试和 `ucar_2026/CMakeLists.txt`；车端目标为动态确认的 `ucar-mini`，未写死 IP。
- **验证**：车端文件 SHA-256 与本地一致；Python2 AST、两个 `roslaunch --nodes` 和车端入口测试 `9/9` 通过；4 个 Python 文件权限为可执行；车端无 `production_task_2026`、`ocr_search`、`ocr_alignment` 或 `roslaunch` 残留进程；未启动 ROS、未发送运动指令。
- **启动**：源码环境加载后执行 `roslaunch ucar_2026 ocr_alignment.launch` 验证连续对齐；完整搜索执行 `roslaunch ucar_2026 ocr_search.launch`。

## 2026-08-20｜OCR 连续对齐 15 秒与独立对齐验证入口（改动完成）

- **目标**：独立 OCR 搜索在候选出现后连续进行视觉对齐，单点最多等待 15 秒，超时继续下一个点；另提供只验证 OCR 对齐并在成功后播报“对齐完成”的独立 launch。
- **改动**：搜索节点以候选帧为起点连续抓帧、按 PD 角速度对齐，单点使用 15 秒墙钟预算，超时跳点；新增 `ocr_alignment.py`/`ocr_alignment.launch`，当前姿态直接验证对齐，成功发布 `ALIGNMENT_COMPLETED` 并调用车端 TTS 播报“对齐完成”；CMake 和两个独立入口测试已同步。
- **验证**：两个独立入口回归 `9/9` 通过；标准 `ucar_2026` 测试 `131` 项通过、`90` 项按环境跳过；两个独立 launch 与原省赛 launch XML 解析通过；两个脚本 Python 语法、`git diff --check` 通过；原省赛 `2026.launch` 与 `production_task_2026.py` 未变化；未启动 ROS、未连接车端、未发送运动指令。
- **风险**：车端首次使用前仍需按操作文档确认车辆静止、`/odom_raw` 有限、相机/OCR helper/TTS 可用；本机未进行真实旋转、OCR 和播报验证。

## 2026-08-20｜独立 OCR 对齐失败后跳过点位（方案已替代）

- **目标**：独立 OCR 调试节点每个点最多进行 5 次 OCR 对齐；5 次仍未对齐时放弃当前点，继续导航到下一个点识别，不再通过增加像素容差放宽成功条件。
- **结果**：该临时的“5 次对齐/第二次 OCR”实现已按用户澄清替换为候选帧起步、连续对齐 15 秒；当前行为以本记录上方的连续对齐方案为准。

## 2026-08-20｜独立 OCR 调试路线简化（改动完成）

- **目标**：将独立 OCR 调试节点改为 `3→428→429...436→445→444→437→419→427`，每个点只做一圈 OCR 搜索；动态障碍占用点直接跳过，全部点无识别结果时导航到 441。
- **改动**：独立路线、动态障碍跳点、无结果前往 441 和 `0.70rad/s` 扫描速度已固定在独立入口；不改原省赛主流程。
- **验证**：与本轮对齐改动一并完成脚本、launch、入口隔离、标准包回归和 `git diff --check` 验证；未启动 ROS、未连接车端、未发送运动指令。
- **风险**：路线和障碍判定仍需车端静止/低速现场验证，正式省赛入口不受本独立节点影响。

## 2026-08-20｜独立 OCR 搜索 launch（改动完成）

- **目标**：参照省赛 `ucar_2026` 主流程，拆出一个默认从 3 号点启动、朝向 13 号点的独立 OCR 搜索节点；只负责导航到 OCR 点并做 360° OCR 搜索，不改变原省赛入口。
- **改动**：新增独立 `ocr_search.py`、`ocr_search.launch`、入口隔离测试/安装声明和运行说明；复用现有底层导航、相机、OCR helper 与省赛 OCR 主路线。默认检查车辆位于 3 号点，原地朝向 13 号点后开始搜索。
- **验证**：本机 Python 语法检查通过；独立/原省赛 launch XML 解析通过；独立入口与省赛感知回归共 `17/17` 通过；`git diff --check` 通过；未启动 ROS、未连接车端、未发送运动指令。
- **风险**：车端首次使用前必须按操作文档确认车辆静止、位于 3 号点附近、`/odom_raw` 有限且两个 TF 正常；本入口只做 OCR 搜索，不包含对准/停车/搬运流程。

## 2026-08-20｜CymPlanner 实车速度死区托底（改动完成）

- **目标**：避免规划器输出低于实车平移 `0.06m/s`、旋转 `0.12rad/s` 死区的非零速度，导致车辆不动而导航误差继续存在。
- **改动**：新增 `min_vel_x`、`min_vel_theta` 参数和符号保持的非零速度托底；普通、终点、横移和恢复平移控制均接入，终点容差内仍保持零速度；补充 6 项边界单测。
- **验证**：车端 Ubuntu 18.04 / ROS Melodic 成功重建 `libcym_planner.so`；规划器单测 `6/6` 通过；4 个源码/参数/测试文件本地与车端 SHA-256 一致；Catkin 白名单已恢复为 `usb_cam`；未启动 ROS、未发送运动指令。
- **风险**：运行中的规划器不会热加载新库和参数；须在车辆安全静止后重启实际导航/2026 主流程，并观察初始化日志中的 `min/max` 速度和到点误差。

## 2026-08-20｜OCR 纠正墙点重新吸附网格并拆分尾向停车（改动完成）

- **目标**：处理实车 448 日志中最终尾向墙边目标卡死的问题，并满足复核纠正后位置重新吸附 0.25m 网格的要求。
- **改动**：三套 2026 主流程保存复核曝光请求时的地图位姿；用该位姿沿雷达朝向与地图墙边界求交并匹配编号墙点。墙点改变时以编号 0.25m 网格墙点重新计算内缩 25cm 停车坐标；墙点不变时保留首次实测连续位置。最终停车拆成车头朝墙导航和原地转为车尾朝挡板两阶段。
- **范围**：三套任务脚本、三套几何回归测试、OCR 变更/操作/现场参数文档及 AI 失败方案与错误记录。
- **验证**：三套任务脚本 Python 编译、`git diff --check` 通过；三套几何测试分别运行 `107`、`123`、`108` 项，均 `OK`，ROS 依赖用例按本机环境跳过 `90`、`106`、`91` 项；三套感知测试均 `14/14`，纠正几何断言通过。三套主流程脚本已同步 `ucar-mini (192.168.8.231)`，本地/车端 SHA-256 分别为 `256d0640...14fdbda5`、`1d711bd5...1d0b2393`、`e6a0548c...8407c33bb`，车端 Python2 AST/无 NUL 通过。未自动启动主流程、未发送运动指令。
- **风险**：本机未加载 ROS Python2，新增任务类回归在本地按设计跳过；车上须安全重启对应 2026 launch 后通过 `PRODUCTION_PROCESSING_CORRECTION`、`processing final approach point` 和 `processing final tail-to-wall rotation` 日志验证现场行为。
