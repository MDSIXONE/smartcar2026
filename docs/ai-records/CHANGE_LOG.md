...
## 2026-08-20｜OCR 内墙 40cm 角度扫读二次确认（改动完成）

- 目标：首次 OCR 识别后到达墙内 40cm，在墙面法向 `+45°` 到 `-45°` 范围内连续扫读并再次确认类别。
- 范围：三套 2026 主流程、三套 launch 参数、几何停车回归测试和运行说明。
- 验证：三套感知测试均 `14/14`；三套几何测试分别 `106/106`、`122/122`、`107/107`，按环境跳过 `89`、`105`、`90` 个 ROS 用例；三套任务脚本 AST、三份 launch XML 和 `git diff --check` 通过。车端尚未重启主流程，未发送运动指令。
