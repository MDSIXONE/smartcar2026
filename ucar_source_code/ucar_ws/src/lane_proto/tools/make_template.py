...
触发区结构(三层, 全部按【当前行车道半宽的比例】给, 不是固定像素):
  死区   tol  (默认 8%  半宽): 线在此范围内摆动不修正 —— 消抖
  触发带 band (默认 30% 半宽): 线侵入多少就修正多少, 侵满打满舵
  评估行 ylo..1.0        : 只看近场(消失点到底边的下 55%),
                           远处一个像素对应好几厘米, 分割抖一下就误触发
  按比例给是关键: 车道半宽在图像里近宽远窄, 而"横向偏了 2cm"这件事
  在每一行对应的像素数不同; 除以当前行半宽后, 各行阈值才代表同一个
  真实横向偏差(约 tol*210mm)。红区因此是收敛于消失点的楔形。
...
参数: --tol 0.08 --band 0.30 --ylo 0.45 --slope 1.33 --vy 71
"""
import argparse
import os
import sys
...
def build(axis, k, vy, tol, band, ylo, out):
    ...
    for i, y in enumerate(ys):
        ...
        if h <= 2:
            continue
        # 左: 从标称线内侧 tol*h 起, 宽 band*h
        l0 = int(round(axis - h * (1.0 - tol)))
        l1 = int(round(axis - h * (1.0 - tol - band)))
        r0 = int(round(axis + h * (1.0 - tol - band)))
        r1 = int(round(axis + h * (1.0 - tol)))
        if l1 > l0:
            img[y, max(0, l0):max(0, l1)] = RED
        if r1 > r0:
            img[y, min(IN_W, r0):min(IN_W, r1)] = RED
    ...
    print("评估行 y=%d..%d  死区 %.0f%% 半宽  触发带 %.0f%% 半宽"
          % (y0, y1, tol * 100, band * 100))
    print("  (420mm 车道: 死区 ≈%.0fmm, 侵满 ≈%.0fmm 横向偏差)"
          % (tol * 210, (tol + band) * 210))
    print("->", out)
    return img
...
def main():
    ...
    ap.add_argument("--band", type=float, default=0.30, help="触发带(半宽比例)")
    ...
    a = ap.parse_args()
    assert a.tol + a.band < 0.95, "tol+band 必须 <1, 否则左右带会越过中线"
        ...
        if a.no_symmetric:
            ...
            if abs(d) > 25:
                print("!! 偏差偏大: 采样帧里车可能没摆正(或相机真的装歪),"
                      " 建议重采更正的帧再跑一次")
    img = build(axis, k, vy, a.tol, a.band, a.ylo, a.out)
