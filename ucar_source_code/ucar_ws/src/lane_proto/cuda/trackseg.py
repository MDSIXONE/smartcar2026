...
# 同一进程内按路径缓存已加载的库。.so 里的权重/显存缓冲是**进程级全局**的
# (ts_init 里有 g_ready 守卫), 所以多个 TrackSeg 实例本来就共用一套, 不会
# 重复加载; 这里缓存只是省掉重复的 dlopen 记账和 ts_init 调用。
_LIBS = {}
...
class TrackSeg(object):
    def __init__(self, lib_path=None):
        ...
        # 同样的道理: 显式给的路径转成绝对路径, 免得 dlopen 去系统目录找
        lib_path = os.path.abspath(lib_path) if lib_path else lib_path
    ...
    def close(self, force=False):
        """⚠ 释放的是**进程级**的权重和缓冲, 不是这个实例私有的 ——
        同进程里别的 TrackSeg 实例会跟着一起失效(再调 infer 会报
        "call ts_init first")。正常用不着调: 进程退出时 OS 全收走。
        真要提前放掉显存(比如临时给 yolo 腾地方)才 force=True。"""
        if not force:
            return
