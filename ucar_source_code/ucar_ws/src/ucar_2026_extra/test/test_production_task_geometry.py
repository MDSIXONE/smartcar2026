...
class ProductionTaskOcrProfileValidationTest(unittest.TestCase):
    ...
    def test_launch_profile_yaml_is_a_direct_list_root(self):
        profile_path = os.path.join(
            PACKAGE_ROOT, "config", "ocr_route_profile.yaml")
        with open(profile_path, "r") as profile_file:
            meaningful_lines = [
                line.strip() for line in profile_file
                if line.strip() and not line.lstrip().startswith("#")]
        self.assertEqual(meaningful_lines, ["[]"])
