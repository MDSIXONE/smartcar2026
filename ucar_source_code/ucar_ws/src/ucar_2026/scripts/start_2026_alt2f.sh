#!/usr/bin/env bash
# Real-vehicle launcher for alt2f.  Same network/master
# management as start_2026.sh, but the mission mode launches
# 2026_alt2f.launch (alt2 8 方位 + OCR 逐簇对准).
# 用法: bash start_2026_alt2f.sh <仿真电脑_IP> [mission|check]  (默认 mission)
# 停止: 在本终端 Ctrl-C（stop_2026_task.sh 的 pgrep 不匹配该 launch 名）。
set -eo pipefail
...
case "$launch_mode" in
  ...
  mission)
    ...
    echo "正在启动自动生产任务（alt2f：方案二 8 方位 + OCR 逐簇对准）……"
    echo "任务节点启动后会等待语音指令：请说“小飞小飞”，再完整说出任务命令。"
    run_launch ucar_2026 2026_alt2f.launch task_enabled:=true \
      simulation_host:="$SIMULATION_HOST"
    ;;
  *)
    echo "错误：start_2026_alt2f 启动器模式只能是 check 或 mission，当前为 '$launch_mode'。" >&2
    exit 2
    ;;
esac
