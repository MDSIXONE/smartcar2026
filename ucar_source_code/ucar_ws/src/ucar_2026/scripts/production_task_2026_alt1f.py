...
alt1f = 方案一（alt1）+ OCR 逐簇修复：读取 live_ppocr helper 的逐框
detection["items"]，按邻近 union-find 聚成“每牌一簇”；转圈候选逐簇
归类，对准只跟目标类别所在簇——两牌同框不再并成一块假牌。
除此之外与 alt1 逐字节一致：15s 对准墙钟预算、发散仅重置 PD 导数、
空帧边转边抓等 alt1 语义原样保留；不含 alt3 的 A2 导航跳过改动。
...
if __name__ == "__main__":
    rospy.init_node("production_task_2026_alt1f")
    try:
        ProductionTask2026()
    except TaskDefinitionError as exc:
        rospy.logfatal("Invalid production task configuration: %s", exc)
        raise
    rospy.loginfo("2026 production task (alt1f) node started.")
    rospy.spin()
