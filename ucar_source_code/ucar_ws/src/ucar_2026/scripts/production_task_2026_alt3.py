...
省赛备用方案三（alt3）：复制自 production_task_2026.py，主流程完全相同，
仅两处失败语义降级 + OCR 逐框（items）按牌聚类：
  A2 目标不可达跳过——主路线直达腿的全局规划失败/导航超时不再
     MissionAbort，而是与守卫命中同路：先试守卫角点 fallback，再把该点
     标记 ignored 继续任务；新增 route_goal_timeout（默认 0 = 沿用 goal_timeout）。
  A3 OCR 逐簇对准——读取 live_ppocr helper 返回的 detection["items"] 逐框结果，
...
class ProductionTask2026(object):
    def __init__(self):
        ...
        # alt3 (A2): optional tighter per-leg goal timeout for route
        # targets only; 0 keeps the normal goal_timeout.  A blocked
        ...
        if (not is_finite(self.route_goal_timeout) or
                self.route_goal_timeout < 0.0):
            raise TaskDefinitionError(
                "route_goal_timeout must be finite and non-negative")
    ...
    def navigate_target_and_scan(
        ...
        if fallback_navigation:
            leg_goal_timeout = self.target_guard_fallback_timeout
        elif self.route_goal_timeout > 0.0:
            leg_goal_timeout = self.route_goal_timeout
        else:
            leg_goal_timeout = None
        reached = self.navigate_coordinates(
            ...
            # alt3 (A2): a blocked route target must never end the
            # mission.  Plan/goal failures degrade to a skip; the
            # grouped scheduler then tries clear corner poses and
            # finally marks the point ignored, exactly like a
            # guard-triggered skip.
            abort_on_navigation_failure=False,
            goal_timeout=leg_goal_timeout,
        ...
        if not reached:
            ...
            # alt3 (A2): an unplannable or unreachable route target
            # skips instead of aborting the whole mission.
            self.last_target_guard_fallback_candidates = (
                self.target_guard_fallback_candidates(
                    end_number, monitor))
            self.record_target_navigation_skip(
                leg_index, start_number, end_number, route_name,
                next_target_number)
            return "target_navigation_failed"
    ...
    def cruise_grouped_production_route(
        ...
        while True:
            ...
            outcome = self.navigate_target_and_scan(
                ...
                route_leg_count=len(self.production_route_numbers),
                next_target_number=next_point)
            if outcome in ("target_guard_skipped",
    ...
    def record_target_navigation_skip(
            self, leg_index, start_number, end_number,
            route_name="primary", next_target_number=None):
        """alt3 (A2): audit one navigation-failure skip like a guard skip."""
        self.target_scan_events.append({
            "timestamp": time.strftime("%Y-%m-%dT%H:%M:%S%z"),
            "target_point_number": int(end_number),
            "outcome": "target_navigation_skipped",
            "route_name": str(route_name),
            "segment_index": int(leg_index),
            "segment_start_point_number": int(start_number),
            "next_target_point_number": next_target_number,
        })
        self.publish_state("TARGET_NAV_SKIP_%03d" % end_number)
        rospy.logwarn(
            "PRODUCTION_TARGET_NAVIGATION_SKIP route=%s target=%d "
            "next_target=%s (alt3: plan/goal failure degrades to skip)",
            route_name, end_number,
            (str(next_target_number)
             if next_target_number is not None else "none"))
        self.save_observation_summary()
    ...
    def observe_wall(
        ...
        for attempt in range(1, self.ocr_alignment_attempts + 1):
            ...
            if divergence_count >= 2:
                # alt3 (A3): a diverging alignment abandons this
                # candidate instead of aborting the mission; the
                # enclosing scan records it as rejected and keeps
                # turning.
                rospy.logwarn(
                    "PRODUCTION_OCR_ALIGNMENT_DIVERGED point=%d "
                    "attempt=%d/%d; giving up this candidate (alt3)",
                    route_point_number, attempt,
                    self.ocr_alignment_attempts)
                aligned = False
                break
        ...
        except MissionAbort as ranging_error:
            # alt3 (A3): a failed range/wall match rejects this
            # candidate instead of ending the mission.  Real sensor
            # loss still trips the safety gate on the very next
            # control loop elsewhere.
            rospy.logwarn(
                "PRODUCTION_OCR_RANGING_REJECTED point=%d error=%s "
                "(alt3)", route_point_number, ranging_error)
            return observation
        observation.update({
...
if __name__ == "__main__":
    rospy.init_node("production_task_2026_alt3")
    try:
        ProductionTask2026()
    except TaskDefinitionError as exc:
        rospy.logfatal("Invalid production task configuration: %s", exc)
        raise
    rospy.loginfo("2026 production task (alt3) node started.")
    rospy.spin()
