...
case "$launch_mode" in
  ...
  mission)
    ...
    echo "任务节点启动后会等待语音指令：请说“小飞小飞”，再完整说出"
    echo "“前往物品领取区，取得日用品，放置在对应仓库，并领取仿真环境中需要的食品放置在对应仓库”。"
    echo "语音填写的是现实/仿真物品的加工类别（食品、日用品或电子产品，必须不同）；"
    echo "任务会在二维码阶段解析实际物品名，再继续生产和仿真联动。"
    exec "$python2_runner" /opt/ros/melodic/bin/roslaunch \
