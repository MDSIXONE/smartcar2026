...
alt2f = 方案二（alt2，8 方位定点扫描版）+ OCR 逐簇修复：读取 helper
的逐框 detection["items"]，按邻近 union-find 聚成“每牌一簇”；候选
逐簇归类，对准只跟目标类别所在簇——两牌同框不再并成一块假牌。
除此之外与 alt2 逐字节一致（8 方位扫描、对准预算等语义原样保留）；
不含 alt3 的 A2 导航跳过改动。另修正了 alt2 里 init_node 误写为
alt1 的节点名字符串（alt2 原文件未改动）。

省赛备用方案二：基于备用方案一（production_task_2026_alt1.py）复制。与 alt1
唯一的差别是到达每个目标点后的 OCR 扫描方式：不再连续旋转一整圈，而是按固定
方位数（默认 8 个）逐一停靠扫描——每个方位转到位后停约 1 秒抓帧，发现候选后
执行与 alt1 一致的连续对齐（固定 15s 墙钟预算、固定 30px 容差、发散两次仅重置
PD 导数继续、空检测继续边转抓帧）；对齐失败继续扫描剩余方位。导航/QR/巡线交接/
对齐后测距与停车坐标决定/任务节点直发 /cmd_vel 等其余逻辑与 alt1 完全一致。
...
Flow:
  ...
  5. Navigate the grouped production targets.  The forward pass tries one
     ...
     points that were not tried.  After arrival, scan a fixed number of
     headings (default 8): each heading stops about 1 s while one OCR frame
     is captured; a candidate stops the turn, passes a fresh-odometry stop
     gate, then aligns the box and reads the front lidar distance.  An
     alignment failure continues with the remaining headings.
...
def _ocr_item_bbox(item):
    """[left, top, width, height] from one live_ppocr per-box item quad."""
    quad = item.get("quad") if isinstance(item, dict) else None
    if not quad:
        return None
    try:
...
def cluster_ocr_items(items, gap_scale=0.8):
    """Group per-box OCR items into per-sign clusters by proximity.

    Two boxes join one cluster when their bboxes, each expanded by
    ``gap_scale`` x its own height on every side, intersect.  One sign's
    text lines sit within about one line height of each other, while two
    signs on the field are separated by far more than that, so the rule
    needs no per-layout tuning.  Members are ordered top row first, then
    left to right, so the concatenated text keeps reading order.  Pure
    Python 2/3 with no ROS imports so it stays unit-testable off-vehicle.
    """
    boxes = []
    for item in items or []:
        bbox = _ocr_item_bbox(item)
        if bbox is None or bbox[2] <= 0.0 or bbox[3] <= 0.0:
            continue
        text = item.get("text") or u""
    ...
    def expanded(box):
        ...
        margin = gap_scale * height
        return (left - margin, top - margin,
                left + width + margin, top + height + margin)

    for first in range(len(boxes)):
        first_rect = expanded(boxes[first])
        for second in range(first + 1, len(boxes)):
            second_rect = expanded(boxes[second])
            if (first_rect[0] <= second_rect[2] and
                    second_rect[0] <= first_rect[2] and
                    first_rect[1] <= second_rect[3] and
                    second_rect[1] <= first_rect[3]):
                union(first, second)
    ...
    for members in grouped.values():
        heights = sorted(member["bbox"][3] for member in members)
        row_height = max(1.0, heights[len(heights) // 2])
        members.sort(key=lambda member: (
            int(round((member["bbox"][1] + member["bbox"][3] / 2.0) /
                      row_height)),
            member["bbox"][0]))
        left = min(member["bbox"][0] for member in members)
        top = min(member["bbox"][1] for member in members)
        right = max(member["bbox"][0] + member["bbox"][2]
                    for member in members)
        bottom = max(member["bbox"][1] + member["bbox"][3]
                     for member in members)
        clusters.append({
            "items": members,
            "text": u"".join(member["text"] for member in members),
...
def _scored_production_clusters(detection, image_width, min_area_px=0.0):
    """Per qualifying cluster: (category, synthetic_detection, offset).

    Returns None when the helper supplied no per-box items at all, so the
    caller can fall back to the legacy merged whole-frame detection.
    """
    if not isinstance(detection, dict):
        return []
    clusters = cluster_ocr_items(detection.get("items"))
    if not clusters:
        return None
    centre = float(image_width) / 2.0
    scored = []
    for cluster in clusters:
        category = normalize_production_category(cluster["text"])
        if category is None:
            continue
        left, _top, width, height = cluster["bbox"]
        if float(min_area_px) > 0.0 and width * height < float(min_area_px):
            continue
        offset = abs((left + width / 2.0) - centre)
        scored.append((category, {
            "text": cluster["text"],
            # Same 0-100 convention as the merged detection confidence.
            "confidence": cluster["confidence"] * 100.0,
...
def production_cluster_candidates(detection, image_width, min_area_px=0.0):
    """Ordered [(category, detection)], best per-sign cluster per category.

    alt3 (A3): a frame that shows two signs yields two separate candidates
    instead of one merged pseudo-sign.  Falls back to the merged detection
    only when the helper supplied no per-box items (older helper builds).
    """
    scored = _scored_production_clusters(detection, image_width, min_area_px)
    if scored is None:
        category = normalize_production_category(
            detection.get("text") if isinstance(detection, dict) else None)
        if category is None:
            return []
        return [(category, detection)]
    best_by_category = {}
    for category, synthetic, offset in scored:
        current = best_by_category.get(category)
        if (current is None or
                (-synthetic["confidence"], offset) <
                (-current[0]["confidence"], current[1])):
            best_by_category[category] = (synthetic, offset)
    ordered = sorted(
        best_by_category.items(),
        key=lambda entry: (-entry[1][0]["confidence"], entry[1][1]))
    return [(category, synthetic)
            for category, (synthetic, _offset) in ordered]


def select_category_cluster(detection, image_width, category):
    """Synthetic detection for one category's best per-sign cluster.

    alt3 (A3): the alignment loop steers only on the requested sign's own
    cluster; None when that sign is not in this frame.  Falls back to the
    merged detection when the helper supplied no per-box items and the
    merged text itself names the category.
    """
    scored = _scored_production_clusters(detection, image_width)
    if scored is None:
        merged_category = normalize_production_category(
            detection.get("text") if isinstance(detection, dict) else None)
        return detection if merged_category == category else None
    matching = [
        (synthetic, offset)
        for cluster_category, synthetic, offset in scored
        if cluster_category == category]
    if not matching:
        return None
    matching.sort(key=lambda entry: (-entry[0]["confidence"], entry[1]))
    return matching[0][0]
...
class ProductionTask2026(object):
    def __init__(self):
        ...
        if self.ocr_scan_positions <= 0:
            raise TaskDefinitionError("ocr_scan_positions must be positive")
        if (not is_finite(self.ocr_scan_dwell_seconds) or
                self.ocr_scan_dwell_seconds <= 0.0):
            raise TaskDefinitionError(
                "ocr_scan_dwell_seconds must be positive")
        ocr_scan_values = (
    ...
    def scan_production_point(
            ...
            target_category=None, record_categories=None):
        """Complete one stationary fixed-heading scan and record new classes.

        ``record_categories`` limits which categories may be persisted.  The
        scan stops early only after every category in ``record_categories``
        has been recorded.  Otherwise all configured headings (default 8) are
        visited; ``target_category`` only identifies the category hunted by
        the enclosing cruise.  A (category, wall_point_number) pair already in
        served_wall_points is never recorded again, so a category can be
        stopped at once per wall.  An alignment failure for a candidate
        continues with the remaining headings instead of abandoning the point.
        """
        ...
        try:
            ...
            def handle_candidate(response, turn_progress):
                merged_detection = response["detection"]
                # alt3 (A3): split the frame into per-sign clusters from
                # the helper's per-box items; a frame that shows two signs
                # must not vote as one merged pseudo-sign.
                cluster_candidates = production_cluster_candidates(
                    merged_detection,
                    int(response.get("width", self.camera_width)),
                    self.ocr_candidate_min_bbox_area_px)
                category = None
                detection = None
                for candidate_category, candidate_detection in \
                        cluster_candidates:
                    if (record_categories is not None and
                            candidate_category not in record_categories):
                        rospy.loginfo(
                            "PRODUCTION_CATEGORY_IGNORED category=%s "
                            "record_categories=%s route_point=%d",
                            candidate_category.encode("utf-8"),
                            self.log_safe_text(sorted(record_categories)),
                            point_number)
                        continue
                    if candidate_category in rejected_categories:
                        rospy.loginfo(
                            "PRODUCTION_CATEGORY_SKIP_RETRY category=%s "
                            "route_point=%d reason=alignment_rejected",
                            candidate_category.encode("utf-8"), point_number)
                        continue
                    if candidate_category in handled_categories:
                        rospy.loginfo(
                            "PRODUCTION_CATEGORY_SKIP_TURN_DUPLICATE "
                            "category=%s route_point=%d",
                            candidate_category.encode("utf-8"), point_number)
                        continue
                    if (candidate_category != target_category and
                            self.production_category_recorded(
                                candidate_category)):
                        rospy.loginfo(
                            "PRODUCTION_CATEGORY_SKIP_RECORDED category=%s "
                            "route_point=%d text=%s",
                            candidate_category.encode("utf-8"), point_number,
                            json.dumps(candidate_detection.get("text"),
                                       ensure_ascii=True))
                        continue
                ...
                observation = self.observe_wall(
                    point_number, observation_label,
                    target_text_category=category)
                observation.update({
            ...
            self._ocr_turn_stop_flag = False
            _response, turn_progress = self.scan_ocr_positions(
    ...
    def scan_ocr_positions(self, label, candidate_handler=None):
        """Scan a fixed number of headings instead of one continuous 360° turn.

        The scan keeps turning in one direction and uses accumulated actual
        turn progress (positive_turn_increment) to decide when a heading is
        reached.  At each heading the chassis stops and dwells for
        ``ocr_scan_dwell_seconds`` while one OCR frame is captured.  A strong
        candidate is passed to the handler (which stops, aligns, ranges and
        records it); any handled candidate that fails alignment simply
        continues with the remaining headings.
        """
        self.move_base.cancel_all_goals()
        self.stop_motion()
        self.wait_for_chassis_stop(label + " start")
        ...
        timeout = (2.0 * math.pi / speed * self.rotation_timeout_scale +
                   self.ocr_scan_positions * self.ocr_scan_dwell_seconds + 2.0)
        deadline = rospy.Time.now() + rospy.Duration(timeout)
        rate = rospy.Rate(self.rotation_control_rate)
        rospy.loginfo(
            "PRODUCTION_OCR_POSITIONS_START label=%s positions=%d "
            "speed=%.3f dwell=%.1f timeout=%.1f",
            label, self.ocr_scan_positions, speed,
        ...
        try:
            for position_index in range(1, self.ocr_scan_positions + 1):
                ...
                if is_navigation_ocr_candidate(
                        ...
                        self.ocr_candidate_min_bbox_area_px):
                    rospy.loginfo(
                        "PRODUCTION_OCR_POSITION_CANDIDATE label=%s "
                        "position=%d/%d progress=%.3f text=%s confidence=%.1f",
                        label, position_index, self.ocr_scan_positions,
                        progress,
                        json.dumps(response["detection"]["text"],
                                   ensure_ascii=True),
                    ...
                    self.discard_unmatched_motion_frame(response)
            rospy.loginfo(
                "PRODUCTION_OCR_POSITIONS_COMPLETE label=%s positions=%d "
                "progress=%.3f",
                label, self.ocr_scan_positions, progress)
            return None, progress
        finally:
            self.stop_motion()
        raise MissionAbort(
    ...
    def observe_wall(
            ...
            candidate_wall_points=None, target_text_category=None):
        ...
        while not rospy.is_shutdown() and time.time() < deadline:
            ...
            detection = response.get("detection")
            if detection is not None and target_text_category is not None:
                # alt3 (A3): steer only on the requested sign's own
                # cluster; other text in the frame must not move the
                # error signal.  A frame without that sign is handled
                # exactly like an empty OCR frame below.
                detection = select_category_cluster(
                    detection, image_width, target_text_category)
...
if __name__ == "__main__":
    rospy.init_node("production_task_2026_alt2f")
    try:
        ProductionTask2026()
    except TaskDefinitionError as exc:
        rospy.logfatal("Invalid production task configuration: %s", exc)
        raise
    rospy.loginfo("2026 production task (alt2f) node started.")
    rospy.spin()
