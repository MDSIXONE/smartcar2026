#!/usr/bin/env bash
# Real-vehicle launcher for alt1f.  Same network/master
# management as start_2026.sh, but the mission mode launches
# 2026_alt1f.launch (alt1 + OCR 逐簇对准).
# 用法: bash start_2026_alt1f.sh <仿真电脑_IP> [mission|check]  (默认 mission)
...
case "$launch_mode" in
  ...
  mission)
    ...
    echo "正在启动自动生产任务（alt1f：方案一 + OCR 逐簇对准）……"
    echo "任务节点启动后会等待语音指令：请说“小飞小飞”，再完整说出任务命令。"
    run_launch ucar_2026 2026_alt1f.launch task_enabled:=true \
      simulation_host:="$SIMULATION_HOST"
    ;;
  *)
    echo "错误：start_2026_alt1f 启动器模式只能是 check 或 mission，当前为 '$launch_mode'。" >&2
    exit 2
    ;;
esac
