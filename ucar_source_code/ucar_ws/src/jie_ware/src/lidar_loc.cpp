...
void crop_map()
{
    ...
    // Apply the launch-configured initial pose once the map crop offset is known.
    // Subsequent RViz /initialpose messages override it without being reset by map updates.
    if (!initial_pose_initialized)
    {
        geometry_msgs::PoseWithCovarianceStamped init_pose;
        init_pose.header.frame_id = "map";
        init_pose.pose.pose.position.x = initial_pose_x;
        init_pose.pose.pose.position.y = initial_pose_y;
        tf2::Quaternion initial_orientation;
        initial_orientation.setRPY(0.0, 0.0, initial_pose_a);
        init_pose.pose.pose.orientation = tf2::toMsg(initial_orientation);
