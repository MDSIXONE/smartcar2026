...
waypoints_dict = {
    "nothing":(),
    "point-": (1.650, 0.250, 0.000, 1.000),  #   
    "point1": (1.650, 0.430, 0.707, 0.707),  # 
    "poinxx": (1.650, 0.630, 1.000, 0.000),  # 
    "point2": (0.790, 0.630, 1.000, 0.000),  # 二维码识别点 
    "point3": (0.1, 0.970, 1.000, 0.000),  #
    "point3_middle": (0.1, 0.970, 0.000, 1.000), 
    "point4": (1.750, 1.250, 0.000, 1.000),
    "point5": (1.650, 1.670, 1.000, 0.000),
    "point6": (0.100, 1.670, 1.000, 0.000),
    "point7": (0.850, 3.500, 0.707, 0.707),  #识别货物点
    "point8": (0.850, 3.800, 0.707, 0.707),
    "point_traffic_light_middle":(2.4600, 4.300, 0.00, 1.00),
    "point9": (2.4600, 4.300, 0.707, 0.707),
     "point_crossing1_middleR":(2.4600, 4.300, -0.707,0.707),
    #"point_crossing1_middle":(2.6650, 3.800,-0.707,0.707),
    "point11":(2.4650, 3.600,-0.707,0.707),#路口1
    "point12":(3.600,4.120,0.000,1.000),
    "point13":(4.100,4.200,0.707,0.707),#路灯检测点2
    "point14":(4.150,3.950,0.000,1.000),
    "point15":(4.480,3.740,-0.707,0.707),
}
...
class task_control:
    def __init__(self):
        ...
        rospy.Subscriber("/cmd_vel", Twist, self.cmd_vel_cb)  # 监听速度指令
        rospy.Timer(rospy.Duration(0.1), self.check_rotation_state)  # 10Hz检查状态
    ...
    def report_result(self,msg):
        ...
        print(str(formatted_time)+"   语音输出:"+msg)
    ...
    def goods_detect0(self): #新的检测
         ...
         self.yolo_found_and_acquired_value=self.goods_class_index*3
    ...
    def traffic_detect(self):
        ...
        while(True):
            if(self.traffic_answer==0):
                ...
                index=list(waypoints_dict.keys()).index("point12")
                print("traffic node 2")
                print(index)
                rospy.sleep(1)
                break 
    ...
    def adjust_teb_velocity(self, is_rotating):
        """
        动态调整TEB的旋转速度参数
        :param is_rotating: True表示原地旋转，False表示正常移动
        """
        if is_rotating:
            rospy.set_param("/move_base/TebLocalPlannerROS/max_vel_theta", 1.5)  # 原地旋转提速
            rospy.set_param("/move_base/TebLocalPlannerROS/acc_lim_theta", 0.5)  # 增大角加速度
        else:
            rospy.set_param("/move_base/TebLocalPlannerROS/max_vel_theta", 0.6)  # 恢复默认
            rospy.set_param("/move_base/TebLocalPlannerROS/acc_lim_theta", 0.2)

    def cmd_vel_cb(self, msg):
        self.current_cmd_vel = msg  # 保存最新速度指令

    def check_rotation_state(self, event):
    # 判断是否为原地旋转（线速度接近0且角速度非0）
        is_rotating = (
            abs(self.current_cmd_vel.linear.x) < 0.01 and 
            abs(self.current_cmd_vel.angular.z) > 0.1
        )
        self.adjust_teb_velocity(is_rotating)
done=0
...
if __name__ == '__main__':
    ...
    if do_go_around==1:
        print("环岛")
    for i in range(10):
        task.go_around_island_pub.publish(do_go_around)
        rospy.sleep(0.1)
    index=0
    points_num=15 #导航点数
    price=0
    rospy.sleep(10)
    task.report_result("小车启动完成，等待唤醒")
    ...
    while True: 
        ...
        while True:#index < points_num:
            index=index+1
            pose = list(waypoints_dict.values())[index]#switch(points_list[index])
            point_name=list(waypoints_dict.keys())[index]
            goal=goal_pose(pose)
            done=0
            starttime=time.time()
            ...
            if point_name=="point2":
                ...
                task.go_around_island_pub.publish(do_go_around)#再次通知是否绕环岛！以免启动时丢失？
                task.speed=1#降速
            elif point_name=="point9":
                rospy.sleep(1)
                task.traffic_detect()
                # index=11
            elif point_name=="point11" or point_name=="point15":
