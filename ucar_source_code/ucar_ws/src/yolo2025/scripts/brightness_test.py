...
def find_final_goal(frame,reader):
    ...
    gray_bottom = np.dot(bottom_section[..., :3], 
    [0.1140, 0.5870, 0.2989]
    #[1.0, 0.0, 0.0]
    )
    cv2.imwrite("/home/ucar/ucar_ws/src/yolo2025/launch/ss.png", gray_bottom, [cv2.IMWRITE_PNG_COMPRESSION, 9]) 
    #mean_brightness = np.mean(gray_bottom)    
    #print(mean_brightness)
    for num in range(160, 201, 5):  # 201 是因为 range 不包含结束值
        print(num, end="")
        print(":", end="")
        b_brightness = bright_pixels = np.sum(gray_bottom > num) 
        print(str(b_brightness).rjust(5), end="")
        print(" ", end="")
    print("")
####    
kk=-1   
