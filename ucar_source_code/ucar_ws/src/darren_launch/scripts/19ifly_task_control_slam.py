...
#      (0.587, 0.788,-0.708, 0.706), #返回通道
#      (0.500, 0.080,1.000, 0.011), #入坡点
#      (-0.026, 0.000,1.000, 0.004), #入坡点1
#     #  (0.075, -0.010,1.000, -0.013), #入坡点2
#     #  (-1.645, -0.027,1.000, 0.005), #下坡点
#      (-1.685, -0.079,1.000, 0.010), #下坡点
#      (-2.401, -1.921,-0.721, 0.692), #急救包识别
#      (-1.740, 0.798,0.674, 0.739), #救援物品点1
#      (-1.685, -0.921,-0.721, 0.692), #救援物品点2
#     #  (-1.612, 0.011,0.002, 1.000), #返回入坡点
#      (-1.712, 0.000,0.002, 1.000), #返回入坡点
#      (0.034, -0.1720,-0.715,0.699),#返回巡线
#      #(0.031, 0.015,0.007,1.000),#返回下坡点
#      #(1.053, -0.507,0.004, 1.000),#slam通道1
#      #(1.567, -0.307,0.710, 0.704),#slam通道2
#      #(2.091, 0.028,-0.023, 1.000) #入库点
#      (1.925, -0.023,0.694, 0.720)#巡线入库点
...
waypoints_dict = {
    ...
    "point6": (-1.685, -0.079, 1.000, 0.000),  # 下坡点
    "point7": (-2.401, -1.921, -0.707, 0.707),  # 急救包识别
    ...
    "point11": (-1.82, -0.200, 0.000, 1.000),  # 入坡点
    "point12": (0.000, 0.000, 0.000, 1.000),  # 返回Slam
    "point13": (2.000, 0.000, 0.000, 1.000),  # Slam点1
    "point14": (2.000, 0.000, 0.000, 1.000),  # Slam点2   
    "point15": (2.000, 0.000, 0.000, 1.000)  # Slam入库点
}
            ...
            if obj in labels:
                ...
                for i in range(10):
                    ...
                    if abs(x_center - 320) < 20  or j==5 or i==9:
                        ...
                        angle_offset = max(-25, min(25, angle_offset))  # 限幅
                ...
                while index < 15:
                    ...
                    if index == 1:                       
                        ...
                        try:
                            ...
                            if task.recognize_and_ajust(6, 45):#(6,-45)顺时针旋转45
                                pose = swap_last_two(pose)
                                goal=goal_pose(pose)
                                client.send_goal(goal)
                                client.wait_for_result()
                                index = 10
                        ...
                        try:
                            if task.recognize_and_ajust(6,-45):#(6,+45)逆时针旋转45
                                pose = swap_last_two(pose)
                                goal=goal_pose(pose)
                                client.send_goal(goal)
                                client.wait_for_result()
