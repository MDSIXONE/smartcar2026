...
def judge_vegetation_room(first,second):
    ...
    cucumber_number = 0
    max_number = 0
    file_path = "/home/ucar/ucar_ws/src/txt_tatal/"+str(first)+"_"+str(second)+".txt"
...
def judge_room():
    ...
    ## 判断B房间
    result[0], find_index = judge_vegetation_room(1,4)
    if find_index != -1:
        room_find_flag[find_index] = 1
    ## 判断C房间
    result[1], find_index = function_judge(1,3)
    if find_index != -1:
        room_find_flag[find_index] = 1
    ## 判断D房间
    result[2], find_index = function_judge(1,2)
    if find_index != -1:
        room_find_flag[find_index] = 1
    ## 判断E房间
    result[3], find_index = function_judge(1,1)
    if find_index != -1:
        room_find_flag[find_index] = 1
    ## 判断F房间
...
class task_control:
    def __init__(self):
        ...
        self.task_start_sub = rospy.Subscriber("/awake/home/ucar/ucar_ws/src/txt_tatal/_flag", Int8 , self.callback_task_start)
