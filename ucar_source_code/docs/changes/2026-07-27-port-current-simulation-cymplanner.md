...
- `ucar_ws/src/ucar_2026/package.xml`: declare the migrated local planner as a direct runtime
  dependency of the new formal 2026 launch package.
- `README.md`, `config/README.md`, and `docs/operations.md`: document the new control path,
  `ucar_2026` deployment files, build/test commands, dynamic ROS Master setup, and pre-motion
  checks. The historical `yolo2025` package remains separate and is not modified by this port.
...
- No files are deployed to the physical vehicle and no motion command is sent by this migration.
...
- The formal `ucar_2026` launch and compatibility-only `yolo2025` wrapper remain structurally
  separate; no 2026 resource is copied back into `yolo2025`.
...
the address discovered or configured through `rosmaster/NETWORK_CONFIGURATION.md`.
