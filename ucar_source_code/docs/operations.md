...
for path in files:
    ...
    assert 'ocr_recheck_backoff_m' in text and 'value="0.25"' in text
print('OCR stop/backoff: 0.25m; recheck sweep: -45deg to +45deg')
'@ | python -
~~~
...
<param name="ocr_recheck_backoff_m" value="0.25"/>
...
同一轮 360° 扫描中，一个类别只允许进入一次完整的停车、对准和测距流程；后续重复候选应打印
`PRODUCTION_CATEGORY_SKIP_TURN_DUPLICATE` 并继续旋转。第一轮预记录的仿真类别在第二轮作为
`target_category` 时仍允许第一次正式处理，但处理完成后同一轮后续重复候选必须跳过。
2026-08-20 已将三套任务脚本同步到车端，并在车端 Python2 环境完成上述三类回归验证各 `3/3`；
运行中的任务不会热加载，下一次安全重启 2026 主流程后才会实际使用新脚本。
