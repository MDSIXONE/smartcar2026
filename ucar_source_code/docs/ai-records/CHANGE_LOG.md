...
- 状态：三套主流程已接入停车后墙面法向 `+45°` 到 `-45°` 连续 OCR 扫读，三套 launch 停车内缩参数改为 `0.40m`。
- 验证：感知测试均 `14/14`；几何测试 `106/106`、`122/122`、`107/107`，ROS 环境分别跳过 `89`、`105`、`90` 项；三套 Python AST、三份 launch XML 和 `git diff --check` 通过。车端已确认无主流程运行，待本次同步。
