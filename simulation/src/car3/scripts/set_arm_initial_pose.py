...
class InitialArmPoseSetter:
    def __init__(self):
        ...
        # Keep the generic open default configurable.  Task-three preparation
        # explicitly supplies the reference pickup posture (1.0, fully open)
        # so controller takeover does not add an extra opening movement.
        self.gripper_initial = float(rospy.get_param(
            "~gripper_initial_position",
            rospy.get_param("~gripper_open_position", 1.0),
        ))
    ...
    def _set_initial_configuration(self, warn=True):
        ...
        positions = list(self.arm_positions) + [self.gripper_initial]
        positions.extend(self.gripper_initial * ratio for ratio in MIMIC_JOINTS.values())
        try:
    ...
    def _set_pose_then_stop_controllers(self):
        ...
        for _ in range(max(1, self.controller_command_repeats)):
            ...
            self.gripper_pub.publish(Float64(data=self.gripper_initial))
            rospy.sleep(0.05)
    ...
    def run(self):
        ...
        # The task preparation launch uses direct Gazebo configuration.  Do
        # not briefly start either arm controller here: the first controller
        # activation must be the pickup node's _start_arm_control(), after a
        # target has been recognised and aligned.
        if not self.direct_initial_configuration:
