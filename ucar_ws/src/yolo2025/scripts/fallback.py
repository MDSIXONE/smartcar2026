...
####    
  ...
  # iterations 控制腐蚀次数
    ...
    cmd_vel_msg.angular.z = clamp((imu_degree-imu_degree_avoid_obstacle)*mag) #rot/5000.0
...
def handle(frame,reader):
  ...
  # iterations 控制腐蚀次数
    ...
    if ring_pass==1:#环岛已过！直接采取初赛策略
        cmd_vel_msg.angular.z = 0*rot/3500.0
    if go_around_island==0:
        cmd_vel_msg.angular.z = 0*rot/3500.0
    ...
    if cd>0:
        ...
        if ring_pass==0:             
            ...
            if -kk<cmd_vel_msg.angular.z and cmd_vel_msg.angular.z<kk:
                cmd_vel_msg.angular.z=-0*kk*traffic_result
...
def avoid_obstacle():
    ...
    for i in range(10):   
        ...
        cmd_vel_msg.angular.z =0#clamp((imu_degree-imu_degree_avoid_obstacle)*mag)
...
def imu_callback(data):
    ...
    global imu_degree,imu_degree_avoid_obstacle
    if imu_degree_avoid_obstacle<-9000 and start_flag==0:#如果没有初始化（<-9000） 且没有手动设start_flag为1（正常情况下，start_flag到比赛结尾才会设1）
...
if __name__ == "__main__":
    ...
    while(start_flag_x==0):
        rospy.sleep(0.1)
        continue
    
    
    if  traffic_result==1:#左
        client.send_goal(goal_pose((3.48+0.4, 1.600,-0.707,0.707)))
    else:
        client.send_goal(goal_pose((3.48-0.4, 1.600,-0.707,0.707)))
    client.wait_for_result() 
    client.send_goal(goal_pose(    (3.48, 1.700,-0.707,0.707)))
    client.wait_for_result()  
    start_flag=1
    ...
    while not rospy.is_shutdown():
        ...
        if stage==0:
            if lidar_distance<0.4:#正前方障碍物距离大于0.5m时保持巡线
                stage=1
            #print(555)
            handle(frame,reader)
