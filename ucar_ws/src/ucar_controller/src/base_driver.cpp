...
baseBringup::baseBringup() :x_(0), y_(0), th_(0), serial_gap_warn_(0.35)
{
  ...
  pravite_nh.param("angular_cali",   angular_cali_, 1.0);
  pravite_nh.param("linear_cali",   linear_cali_, 1.0); 
...
void baseBringup::writeLoop()
{
  ...
  while(ros::ok())
  {
    try
    {
      ...
      double vw1 = linear_x - linear_y - angular_z * ((base_shape_a_ + base_shape_b_) / angular_cali_);
      double vw2 = linear_x + linear_y + angular_z * ((base_shape_a_ + base_shape_b_) / angular_cali_);
      double vw3 = linear_x - linear_y + angular_z * ((base_shape_a_ + base_shape_b_) / angular_cali_);
      double vw4 = linear_x + linear_y - angular_z * ((base_shape_a_ + base_shape_b_) / angular_cali_);
      ...
      pack_write_.pack.data.pluse_w1 = -period_/1000.0*vw1*(encode_resolution_/(2.0*Pi*wheel_radius_*linear_cali_));
      pack_write_.pack.data.pluse_w2 =  period_/1000.0*vw2*(encode_resolution_/(2.0*Pi*wheel_radius_*linear_cali_));
      pack_write_.pack.data.pluse_w3 = -period_/1000.0*vw4*(encode_resolution_/(2.0*Pi*wheel_radius_*linear_cali_));
      pack_write_.pack.data.pluse_w4 =  period_/1000.0*vw3*(encode_resolution_/(2.0*Pi*wheel_radius_*linear_cali_));
      //LED value
